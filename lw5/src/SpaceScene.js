...
class SpaceScene {
  ...
  constructor () {
    ...
    SpaceScene.#initGuiOptionHandler()
  ...
  static #initGuiOptionHandler () {
    const gui = new GUI()
    gui.width = 400
    const earthFolder = gui.addFolder('Earth')
    earthFolder.add(EARTH_OPTIONS, 'aroundAxis')
      .name('time to rotate around axis')
      .min(MILLISECONDS_PER_SECOND)
      .max(2 * MILLISECONDS_PER_MINUTE)
      .listen()

    earthFolder.add(EARTH_OPTIONS, 'aroundSun')
      .name('time to rotate around sun')
      .min(MILLISECONDS_PER_SECOND)
      .max(15 * MILLISECONDS_PER_MINUTE)
      .listen()

    earthFolder.add(EARTH_OPTIONS, 'orbit')
    ...
    moonFolder.add(MOON_OPTIONS, 'aroundAxis')
      .name('time to rotate around axis')
      .min(MILLISECONDS_PER_SECOND)
      .max(MILLISECONDS_PER_MINUTE * 2)
      .listen()

    moonFolder.add(MOON_OPTIONS, 'aroundEarth')
      .name('time to rotate around earth')
      .min(MILLISECONDS_PER_SECOND)
      .max(MILLISECONDS_PER_MINUTE * 2)
      .listen()

    moonFolder.add(MOON_OPTIONS, 'orbit')
      .name('radius of orbit')
      .min(300)
      .max(2000)
      .listen()
    ...
    sunFolder.add(SUN_OPTIONS, 'aroundAxis')
      .name('time to rotate around axis')
      .min(MILLISECONDS_PER_SECOND)
      .max(MILLISECONDS_PER_MINUTE * 10)
      .listen()
    sunFolder.open()
  }
